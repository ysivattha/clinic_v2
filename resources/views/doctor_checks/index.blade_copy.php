...
<div class="col-md-2">
<form action="{{url('doctor-check')}}" method="GET">
    <div class="row">
{{__('lb.start_date')}} 
...
<div class="col-md-2">
   ...
   <div class="col-md-2">
    {{__('lb.body_part')}} 
    <select name="hospital_id" id="section" class="chosen-select">
        <option value="">{{__('lb.select_one')}}</option>
            ...
            <tbody>			
                ...
                @foreach($technicals as $t)
                    ...
                    <tr 
                        ...
                        @elseif($t->request_status==9 || $t->request_status==10)style="background: #ffecc7;"
                        @endif
                    >
                        <td>{{$i++}}</td>
                        <td class="text-center">   
                            @if($t->request_status <= 9)
                            <a href="{{url('doctor-check/create', $t->id)}}" title="{{__('lb.detail')}}"
                            > @else 
                            <a href="{{url('doctor-check/detail', $t->id)}}" title="{{__('lb.detail')}}"
                                > @endif
                            {{$request->code}}</a></td>
                        ...
                        <td> @if($t->request_status <= 9)
                            <a href="{{url('doctor-check/create', $t->id)}}" title="{{__('lb.detail')}}"
                            > @else
                          
                            <a href="{{url('doctor-check/detail', $t->id)}}" title="{{__('lb.detail')}}"
                                > @endif{{$t->item_name}}</a></td>
                        <td>{{$t->date}}</td>
                        <td> {{\Carbon\Carbon::createFromFormat('H:i:s',$t->time)->format('h:i A')}}</td>
                      <td>
                        @if($t->request_status==8) {{__('lb.waiting_reading')}} 
                     @elseif($t->request_status==9 || $t->request_status==10) {{__('lb.reading')}} 
